...
function update_prices(ticker, price, lastprices, weights, id)
{
        ...
        if (ticker='SPY')
        {
          ...
          console.log("UPDATE PRICE, SPY:" + spy_price)
        }
...
function streamlivedata(tickers,weights,socketid){
 ...
 var alltickers=[];
  ...
  //request_data
  for (var i=0;i<factor_tickers.length;i++)
  {
  //ir.join(tickers[i])

  alltickers.push(factor_tickers[i]);  
 ...
 for (var i=0;i<alltickers.length;i++)
{
  ...
  ir.join(alltickers[i]);
}

  //do the same for factors
  ...
  console.log('New quote for' + ticker);
