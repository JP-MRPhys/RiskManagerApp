...
function calcoptionhegde(username,portfolioname,lossPercentage,socketid)
{

    console.log("Getting Portfolios to compute the hedge");
    ...
    mysqlconnection.query(sql_query,[username,portfolioname],function(err,results){
      
      if (err) {  
                console.log('Error while obtaining portfolio details' + err);
          ...
          for (var i=0;i<rows.length;i++)
          {
            ...
            if (i==rows.length-1){
              ...
              var data=100;
              console.log(data)
              io.to(socketid).emit('hedgeContract',data);
              //calc the hedge based on the option selected and then return to the socket
...
function obtainoptimizedportfolio(username,portfolioname,lossPercentage,socketid)
{

    console.log("In portfolio optimization");
    console.log(socketid)
      ...
      if (err) {  
          ...
          for (var i=0;i<rows.length;i++)
          {
            ...
            if (i==rows.length-1){
              ...
              portfolio={'tickers': tickers, 'weights': weights, 'socketid': socketid}

              Portfolio_Optmization.optimize_portfolio(portfolio,function(optimal_portfolio){

                //now in the callback
                var socketid=optimal_portfolio.socketid;
                var data=optimal_portfolio.optimizeWeights; 
                io.to(socketid).emit('optimzedweights',data);
...
function calcOptionhedge(portfolio,loss){

  console.log("Calc option hedge")
  console.log(portfolio)
  console.log(loss)
...
io.on('connection',function(socket){
 ...
 // users.slice(users.indexof(socket.id,1)); //updateusername function();
  ...
  socket.on('OptmizerPortfolio', function(data){

    console.log('Optimizer portfolio')
    console.log(data);
    console.log(socket.id);
    var userid=socket.request.user.username;

    obtainoptimizedportfolio(userid,data.portfolioname,data.OptimizationOption,socket.id)

 })

  socket.on('hedgePortfolio', function(data){

    console.log('Hedge portfolio')
    console.log(data);
    console.log(socket.id);
    var userid=socket.request.user.username;

    calcoptionhegde(userid,data.portfolioname,data.loss,socket.id)
